...
public class InteractionListener implements Listener {
    ...
    public void onVillagerInteract(final PlayerInteractEntityEvent event) {
        ...
        if (event.getRightClicked() instanceof Villager villager) {
            ...
            NBTItem nbtItem = ItemUtil.validateUnboundItem(itemInMainHand);
            ...
            if (nbtItem != null) {
                ...
                if (villager.getRecipeCount() == 0) {
                    ...
                    // 设置获得绑定卷轴的数量
                    nbtItem.getItem().setAmount(1);
                    // 检查背包是否有空余位置
                    if (player.getInventory().firstEmpty() != -1) {
                        player.getInventory().addItem(data.getItem());
                    } else {
                        // 如果没有空余位置，则丢弃物品
                        player.getWorld().dropItemNaturally(player.getLocation(), data.getItem());
                    }
            ...
            if (event.getRawSlot() == 2 && BoxedVillagers.getMaxXPReward() > 0) // 2 is trade output slot, ignore everything if no xp rewards are set up
            {
                ...
                if (selected != null && event.getRawSlot() == 2 && selected.getUses() < selected.getMaxUses()) {
                    ...
                    if (event.isShiftClick() && !event.getAction().equals(InventoryAction.NOTHING)) {
                        ...
                        // 遍历玩家的物品栏，计算可用空间
                        ItemStack item = selected.getResult();
                        ...
                        tradeCount = Math.min(tradeCount, (int) Math.ceil((float) totalSpace / item.getAmount()));
                    } else if ((event.isLeftClick() || event.isRightClick()) && event.getAction().equals(InventoryAction.NOTHING)) {
                        // 左键或右键点击且手里有物品
                        return;
                    }
